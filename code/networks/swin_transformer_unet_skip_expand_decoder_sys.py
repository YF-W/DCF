...
class SwinTransformerSys_DS(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def forward(self, x):
        feature, x_downsample = self.forward_features(x)
        x = self.forward_up_features(feature, x_downsample)
        x = self.up_x4(x)

        return x, self.up_x2(feature)
