...
class Generic_UNet(SegmentationNetwork):
    ...
    def forward(self, x):
        ...
        for u in range(len(self.tu)):
            x = self.tu[u](x)
            if x.shape != skips[-(u + 1)].shape:
                x = F.interpolate(x, size=skips[-(u + 1)].shape[2:], mode="trilinear", align_corners=True)
...
def initialize_network(threeD=True, num_classes=2):
    ...
    if threeD:
        ...
        default_dict = default_dict_3D
    else:
        conv_op = nn.Conv2d
        dropout_op = nn.Dropout2d
        norm_op = nn.InstanceNorm2d
        default_dict = default_dict_2D
