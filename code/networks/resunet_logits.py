...
class Decoder_DS(nn.Module):
    def __init__(self, params):
        ...
        self.out_conv = nn.Conv2d(self.ft_chns[1], self.n_class,
                                  kernel_size=3, padding=1)
        ...
        self.out_conv_dp1 = nn.Conv2d(self.ft_chns[1], self.n_class,
                                      kernel_size=3, padding=1)
        self.out_conv_dp0 = nn.Conv2d(self.ft_chns[1] * 2, self.n_class,
                                      kernel_size=3, padding=1)
    ...
    def forward(self, feature, shape):
        ...
        dp0_out_seg = self.out_conv_dp0(x)
        dp0_out_seg = torch.nn.functional.interpolate(dp0_out_seg, shape)

        x0 = torch.nn.functional.interpolate(x0, shape)
        x = self.up4(x, x0)
        dp_out_seg = self.out_conv(x)
...
class UNet_DS(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.conv1x1_1 = nn.Conv2d(512, 256, kernel_size=1)
        self.conv1x1_2 = nn.Conv2d(256, 128, kernel_size=1)
        self.conv1x1_3 = nn.Conv2d(128, 64, kernel_size=1)
        self.conv1x1_4 = nn.Conv2d(64, 32, kernel_size=1)
        self.conv1x1_5 = nn.Conv2d(64, 32, kernel_size=1)
    ...
    def forward(self, x):
        ...
        e0_1 = self.conv1x1_5(e0)
        e1 = self.encoder1(e0)  # torch.Size([4, 64,112
        e1_1 = self.conv1x1_4(e1)
        e2 = self.encoder2(e1)  # torch.Size([4, 128,56
        e2_1 = self.conv1x1_3(e2)
        e3 = self.encoder3(e2)  # torch.Size([4, 256,28
        e3_1 = self.conv1x1_2(e3)
        e4 = self.encoder4(e3)  # torch.Size([4, 512, 14
        e4_1 = self.conv1x1_1(e4)
        features = [e0_1, e1_1, e2_1, e3_1, e4_1]

        dp0_out_seg, dp1_out_seg = self.decoder(
            features, shape)
        return dp0_out_seg, dp1_out_seg
