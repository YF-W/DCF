...
class ConvBNReLU(nn.Module):
    def __init__(self, in_chan, out_chan, ks=3, stride=1, padding=1, *args, **kwargs):
        ...
        self.conv = nn.Conv3d(in_chan,
                              out_chan,
                              kernel_size=ks,
                              stride=stride,
                              padding=padding,
                              bias=False)
        self.bn = BatchNorm3d(out_chan)
...
class Head_feature(nn.Module):
    def __init__(self, in_chan, mid_chan, n_classes, *args, **kwargs):
        super(Head_feature, self).__init__()
    ...
    def get_params(self):
        ...
        for name, module in self.named_modules():
            ...
            elif isinstance(module, BatchNorm3d):
...
class VNet_BFA(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False):
        super(VNet_BFA, self).__init__()
        ...
        self.out_0 = Head_feature(in_chan=16, mid_chan=16, n_classes=1)
        self.out_1 = Head_feature(in_chan=32, mid_chan=16, n_classes=1)
        self.out_2 = Head_feature(in_chan=64, mid_chan=16, n_classes=1)
        self.out_3 = Head_feature(in_chan=128, mid_chan=16, n_classes=1)
    ...
    def forward(self, input):
        ...
        return out_seg, features[4]

if __name__ == '__main__':
    # compute FLOPS & PARAMETERS
    from thop import profile
    from thop import clever_format
    model = VNet_BFA(n_channels=1, n_classes=2, normalization='batchnorm', has_dropout=False).cuda()
    input = torch.randn(4, 1, 112, 112, 80).cuda()
