...
class DeepLab(BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='resnet34', pretrained=True, 
    ...
    def forward(self, x):
        ...
        feature = self.ASSP(x)
        x = self.decoder(feature, low_level_features)
        x = F.interpolate(x, size=(H, W), mode='bilinear', align_corners=True)
        return x
...
class DeepLab_DS(BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='resnet34', pretrained=True, 
                output_stride=16, freeze_bn=False, freeze_backbone=False, **_):
                
        super(DeepLab_DS, self).__init__()
        assert ('xception' or 'resnet' in backbone)
        if 'resnet' in backbone:
            self.backbone = ResNet(in_channels=in_channels, output_stride=output_stride, backbone=backbone, pretrained=pretrained)
            low_level_channels = 64
...
if __name__ == "__main__":
    model = DeepLab(num_classes=1).cuda()
