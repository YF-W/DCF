...
class Decoder_DS(nn.Module):
    ...
    def forward(self, feature, shape):
        ...
        x = self.up2(x, x1)
        x = self.up3(x, x0)
        x = self.up4(x, F.interpolate(x0, mode="bilinear", align_corners=False, scale_factor=2))
        dp0_out_seg = self.out_conv(x)
        return dp0_out_seg, x3
...
class UNet_DS(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        params = {'in_chns': in_chns,
                  'feature_chns': [48, 48, 96, 192, 384],
                  'dropout': [0.05, 0.1, 0.2, 0.3, 0.5],
                  'class_num': class_num,
                  'bilinear': False,
                  'acti_func': 'relu'}
        self.encoder = timm.create_model('repvit_m1.dist_in1k', pretrained=True, features_only=True)
    ...
    def forward(self, x):
        ...
        shape = x.shape[2:]
        feature = self.encoder(x)
        feature = [F.interpolate(i, scale_factor=2, align_corners=False, mode="bilinear") for i in feature]
        dp0_out_seg, feature = self.decoder(feature, shape)
        return dp0_out_seg, feature
